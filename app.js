...
guessRows.forEach((guessRow, guessRowIndex) => {
    ...
    guessRow.forEach((guess, guessIndex) => {
        ...
        guessElement.setAttribute('id', 'guessRow-' + guessRowIndex + '-tile-' + guessIndex)
    ...
    if (currentTile > 4) {
        ...
        .then(json => {
            if (json == 'Entry word not found') {
                ...
                flipTile()
                if (wordle == guess) {
                    showMessage("You did it, you guessed the " + wordle + "!")
                    isGameOver = true
                    return
                } else  {
                    if (currentRow >= 5) {
...
const addColorToKey = (keyLetter, color) => {
    const colorToKeys = document.getElementById(keyLetter)
    colorToKeys.classList.add(color)
}
